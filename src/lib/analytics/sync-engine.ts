...
export async function executePlatformSync(
  ...
  const { data: conn, error: connErr } = await supabase
    ...
    .select('id, company_id, provider, provider_account_id, connection_status')
    .eq('id', connectionId)
    .single()
  ...
  try {
    ...
    // 9. Update platform_connections last_successful_sync_at
    await supabase
      .from('platform_connections')
      .update({
    ...
    // 10. Finalize sync_job as completed
    if (jobId) {
      await supabase
        .from('sync_jobs')
        .update({
          ...
          metadata: { recordsCreated, contentItemsImported }
        })
        .eq('id', jobId)
      ...
      await supabase.from('sync_logs').insert({
        ...
        level: 'info',
        step: 'completion',
        safe_message: `Sync completed successfully. ${recordsCreated} metric points saved, ${contentItemsImported} content items imported.`
      })
    }
    ...
    await logAuditAction({
      ...
      summary: `Historical analytics sync completed for ${provider}: ${recordsCreated} metrics saved.`
    })
